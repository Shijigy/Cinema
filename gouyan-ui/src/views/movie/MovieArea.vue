  ...
  <div>
    ...
    <el-card class="box-card">
      <el-row :gutter="20">
        ...
        <el-col :span="2">
          <el-button type="danger" @click="multipleDelete">批量删除区域</el-button>
        </el-col>
      </el-row>
        ...
        <el-table-column label="操作" width="150">
          <template slot-scope="scope">
            <el-tooltip effect="dark" content="修改电影区域" placement="top" :enterable="false" :open-delay="500">
              <el-button type="primary" icon="el-icon-edit" size="mini" @click="showEditDialog(scope.row.movieAreaId)"></el-button>
            </el-tooltip>
            <el-tooltip effect="dark" content="删除区域" placement="top" :enterable="false" :open-delay="500">
              <el-button type="danger" icon="el-icon-delete" size="mini" @click="deleteMovieAreaById(scope.row.movieAreaId)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    getMovieAreaList() {
      const _this = this;
      axios.get(_this.url + 'sysMovieArea', {params: _this.queryInfo}).then(resp => {
        console.log(resp)
    ...
    addMovieArea(){
      ...
      this.$refs.addFormRef.validate(async valid => {
        ...
        await axios.post(_this.url + 'sysMovieArea', JSON.stringify(_this.addForm)).then(resp => {
          console.log(resp)
    ...
    showEditDialog(id){
      const _this = this
      axios.get(_this.url + 'sysMovieArea/' + id ).then(resp => {
        console.log(resp)
    ...
    editMovieAreaInfo(){
      this.$refs.editFormRef.validate(async valid => {
        ...
        await axios.put(_this.url + '/sysMovieArea', JSON.stringify(_this.editForm)).then(resp => {
          if (resp.data.code !== 200){
    ...
    async multipleDelete(){
      ...
      await axios.delete(_this.url + 'sysMovieArea/' + ids).then(resp => {
        if (resp.data.code !== 200){
    ...
    async deleteMovieAreaById(id){
      ...
      await axios.delete(_this.url + 'sysMovieArea/' + id).then(resp => {
        if (resp.data.code !== 200){
