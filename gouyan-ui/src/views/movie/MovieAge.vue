...
export default {
  ...
  methods: {
    getMovieAgeList() {
      const _this = this;
      axios.get(_this.url + 'sysMovieAge', {params: _this.queryInfo}).then(resp => {
        console.log(resp)
    ...
    addMovieAge(){
      ...
      this.$refs.addFormRef.validate(async valid => {
        ...
        await axios.post(_this.url + 'sysMovieAge', JSON.stringify(_this.addForm)).then(resp => {
          console.log(resp)
    ...
    showEditDialog(id){
      const _this = this
      axios.get(_this.url + 'sysMovieAge/' + id ).then(resp => {
        console.log(resp)
    ...
    editMovieAgeInfo(){
      this.$refs.editFormRef.validate(async valid => {
        ...
        await axios.put(_this.url + '/sysMovieAge', JSON.stringify(_this.editForm)).then(resp => {
          if (resp.data.code !== 200){
    ...
    async multipleDelete(){
      ...
      await axios.delete(_this.url + 'sysMovieAge/' + ids).then(resp => {
        if (resp.data.code !== 200){
    ...
    async deleteMovieAgeById(id){
      ...
      await axios.delete(_this.url + 'sysMovieAge/' + id).then(resp => {
        if (resp.data.code !== 200){
